...
export default new Vuex.Store({
  ...
  actions: {
    ...
    //     .then(() => sdk.getTicker())
    //     .then(tickers => {
    //       const tickers = await tickers.filter(ticker => ticker.currency === currency)
    ...
    updateTickers ({ commit, state }, amount) {
      try {
        if (amount === 0 || amount === state.amount) return
        state.amount = amount
        const tickersCalculated = [...state.tickers.map(ticker => {
